...
> **v0.2.1부터** `pdfjs-dist`는 선택적 peerDependency입니다. HWP/HWPX만 쓴다면 설치 불필요.
...
v0.2.2 보안 강화 (v0.2.1 이후 누적):

- **ZIP bomb 방지** — 압축 해제 크기 100MB 제한, 엔트리 수 500개 제한
...
- **손상 ZIP 경로 순회 차단** — `..` 및 절대경로 엔트리 거부, 파일명 길이 1024 제한
- **MCP 서버 경로 제한** — `.hwp`, `.hwpx`, `.pdf` 확장자만 허용
- **파일 크기 제한** — MCP 서버 및 CLI에서 500MB 상한
- **PDF 리소스 정리** — `doc.destroy()`로 WASM 메모리 릭 방지
- **테이블 메모리 가드** — Pass 1에서 sparse Set 기반 할당, 행 수 10,000개 상한
- **HWP5 섹션 제한** — 최대 100 섹션, 손상 파일에 의한 무한 루프 방지
